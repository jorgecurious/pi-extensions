...
Use `pi-ralph-swarm spawn-phase --run RUN --contract roadmap.json --phase PHASE` when a roadmap has repeatable phase contracts. A phase contract hydrates scout/writer/debugger/verifier specs into ordinary swarm agents; it does not add a new scheduler and does not execute work by itself. Prefer `--dry-run` first, then `--enqueue` when the lead wants Pi/Ralph to deliver the generated prompts. `dependsOn` is enforced at queue-drain time, so blocked prompts remain queued until prerequisites complete. Add `setupNotes` or `verificationEnvironment` to contracts when new worktrees need build artifacts, submodule paths, env vars, or other rerunnable verification setup.
...
- Use dependency-ready queue views before draining phase agents.
- Use taskboard claim leases for self-claimed work; do not edit owned paths unless the task is claimable and claimed.
- Use `dependsOn` only for prerequisite-success ordering; use `parentBlockedTask`/`unblocks` for work that exists to unblock a blocked parent.
- Create implementation tasks with review and worktree requirements when they edit product files.
- Move implementation tasks to `review` with `NEEDS_REVIEW`; a separate reviewer must close them with `DONE`.
- Treat greenlight-required task completion as requiring explicit user approval before `DONE`.
...
For swarm agents, this is enforced by state as well as instruction. New agents record the initial task-file hash, and Pi/CLI completion is blocked unless the task file changed, `## Final Verification` has non-placeholder command, working directory, preserved-artifact, and result values, required artifacts exist, and a passing `swarm_verify_agent` or `pi-ralph-swarm verify-agent` monitor record matches the current task-file hash.
